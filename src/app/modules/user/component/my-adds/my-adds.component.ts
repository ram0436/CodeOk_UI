...
export class MyAddsComponent {
  ...
  ngOnInit() {
    ...
    this.projectService.getProjectByUserId(userId).subscribe((data: any) => {
      this.allCards = data;
      this.activeCards = this.allCards.filter(
        (ad: any) => ad.isApproved == true
      );
    });
  ...
  onTabChanged(event: any) {
    if (event.index == 0) {
      ...
      this.pendingApprovalCards = this.allCards.filter(
        (ad: any) => ad.isApproved == false
      );
  }
